...
function displayData() {
    fetch("/data")
      ...
      .then((data) => {
        // Assuming there's an HTML element with id "data-container" to display the data
        const dataContainer = document.getElementById("top_players");
        dataContainer.innerHTML = ""; // Clear previous data
  
        if (data.length === 0) {
          dataContainer.innerHTML = "No data available.";
        } else {
          // Create an ordered list (<ol>) to display the data
...
function storeInformation(score) {
    const name = user;
    const marks = score;
